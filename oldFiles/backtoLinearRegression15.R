...
trainIdxs = idxs[1:(4*nrow(merged)/5)]
testIdxs = idxs[(4*nrow(merged)/5 + 1):nrow(merged)]
...
for(key in keys(frameHash)){
  df = frameHash[[key]]
  df["baselineProbMatch"] = getProbs(frameHash[["train"]], df, c(baselineDecW), "match")
  df["featuresProbMatch"] = getProbs(frameHash[["train"]], df, c(features), "match")  
  frameHash[[key]] = df
...
baseline = womenAccuracyFrame["baselineProbDecW", "overallErrorRate"]
currentBest = womenAccuracyFrame["featuresProbDecW", "overallErrorRate"]
# currentBest - womenAccuracyFrame[womenAccuracyFrame["logLoss"] < currentBest - 10 ,]

currentBest - womenAccuracyFrame[womenAccuracyFrame["overallErrorRate"] < currentBest ,]["overallErrorRate"]
