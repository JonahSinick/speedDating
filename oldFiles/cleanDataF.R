...
ratingsFake = gsub("$", "Fake", ratings)
df[ratingsFake] = 0
for(ratingIdx in 1:length(ratings)){
  for(iid in unique(df[["iid"]])){
    print(c(ratingIdx,iid))
    ratingsArr = df[df["iid"] == iid,ratings[ratingIdx]]
    fakeRatings = seq(0,0, length.out = length(ratingArr))
    idxs = 1:length(ratingsArr)
    for(idx in idxs){
      otherIdxs = idxs[idxs !=idx]
      otherRatings = ratingsArr[otherIdxs]
      fakeRatings[idx] = sample(otherRatings,1)
    }
    df[df["iid"] == iid,ratingsFake[ratingIdx]] = fakeRatings
  }
}
