...
#   print(i)
#   df[df["iid"] == i,][["numRated"]] = length(unique(df[df["iid"] == i,][["pid"]]))
# }
# 
# 
# df["selfPercTotal"] = 0
...
# 
# for(r in ratings){
#   men[paste(r,"AvgExc",sep="")] = (men[paste(r,"Sum",sep="")] - men[r])/men["numRated"]
#   women[paste(r,"AvgExc",sep="")] = (women[paste(r,"Sum",sep="")] - women[r])/women["numRated"]
# }
# 
# for(r in ratings){
...
# write.csv(men, "~/Desktop/menBeforeSplitting.csv")
# write.csv(women, "~/Desktop/womenBeforeSplitting.csv")
...
men = read.csv("~/Desktop/menBeforeSplitting.csv")
women = read.csv("~/Desktop/womenBeforeSplitting.csv")

drops = c(".id")
men = men[,!(names(men) %in% drops)]
women = women[,!(names(women) %in% drops)]
addCols = function(wave,rat){
  ids = unique(wave[["id"]])
  oldColNum = ncol(wave)
  mean =  mean(wave[[rat]])
  trueName = substring(rat, 1,(nchar(rat) - 4))
  if(rat == "dec"){
    trueName = "decOrig"
  }
  if(rat == "dec"){
  ...
  for(i in ids){
    str = paste(trueName,paste("Rater",toString(i),sep="_"),sep="_")
    wave[str] = 0
    for(j in ids){
      if(i != j){
        ...
        wave[wave["id"] == j,][[str]] = mean
      }
    }
  }
  str = paste(trueName,"PartnersAvgExc",sep="")
  wave[str] = rowSums(wave[0:-oldColNum])/length(ids)
  str2 = paste(trueName,"PartnersAvgInc",sep="")
  wave[str2] = (rowSums(wave[0:-oldColNum]) + wave[rat] - mean)/length(ids)
  return(wave)
}

addIDs = function(wave){
  for(i in unique(wave[["id"]])){
    wave[paste("id",toString(i),sep="_")] = 0
  }
  for(i in unique(wave[["id"]])){
    wave[wave["id"] == i,][[paste("id",toString(i),sep="_")]] = 1
  }
  for(j in unique(wave[["partner"]])){
    wave[paste("partner",toString(j),sep="_")] = 0
  }
  for(j in unique(wave[["partner"]])){
    wave[wave["partner"] == j,][[paste("partner",toString(j),sep="_")]] = 1    
  }
  return(wave)
}
...
for(i in 1:20){
  print(i)
  wave = men[men["wave"] == i,]
  wave = wave[,!(names(wave) == ".id")]
  normedRatings = c("attrNorm",  "sincNorm",  "intelNorm", "funNorm",   "ambNorm", "likeNorm")
  features = c("dec","decNorm", normedRatings)
  for(rat in features){
    print(rat)
    wave = addCols(wave, rat)
  }
  len = length(unique(wave[["id"]]))
  wave = addIDs(wave)
  write.csv(wave, paste("~/Desktop/waves/menWave",paste(toString(i),".csv",sep=""),sep=""))
}


for(i in 1:20){
  print(i)
  wave = women[women["wave"] == i,]
  normedRatings = c("attrNorm",  "sincNorm",  "intelNorm", "funNorm",   "ambNorm", "likeNorm")
  features = c("dec","decNorm", normedRatings)
  for(rat in features){
    print(rat)
  ...
  len = length(unique(wave[["id"]]))
  wave = addIDs(wave)
  write.csv(wave, paste("~/Desktop/waves/womenWave",paste(toString(i),".csv",sep=""),sep=""))
}
