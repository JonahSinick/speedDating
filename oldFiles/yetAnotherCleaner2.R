...
for(r in ratings){
  df[paste(r,"Fake",sep="")] = 0
  df[paste(r,"FakePartner",sep="")] = 0
}
...
for(r in ratings){
  for(iid in unique(men[["iid"]])){
    print(r)
    print(iid)
    men[men["iid"] == iid,][[paste(r,"Fake",sep="")]] = sample(men[men["iid"] == iid,][[r]])
  }
  for(iid in unique(women[["iid"]])){
    print(r)
    print(iid)
    women[women["iid"] == iid,][[paste(r,"Fake",sep="")]] = sample(women[women["iid"] == iid,][[r]])
  }
}
...
for(r in ratings){
  ...
  men[paste(r,"FakeNorm",sep="")] = men[paste(r,"Fake",sep="")] - men[paste(r,"Avg",sep="")]
  women[paste(r,"Norm",sep="")] = women[r] - women[paste(r,"Avg",sep="")]
  women[paste(r,"FakeNorm",sep="")] = women[paste(r,"Fake",sep="")] - women[paste(r,"Avg",sep="")]
}

for(r in ratings){
  men[paste(r,"AvgExclud",sep="")] =  (men[paste(r,"Sum",sep="")] - men[r] + men[paste(r,"Fake",sep="")])/men["numRated"]
  women[paste(r,"AvgExclud",sep="")] = (women[paste(r,"Sum",sep="")] - women[r] + women[paste(r,"Fake",sep="")])/women["numRated"]
}

men = ddply(men, .(pid) ,transform ,decPartnerSum = sum(decNorm))
men = ddply(men, .(pid) ,transform ,attrPartnerSum = sum(attrNorm))
men = ddply(men, .(pid) ,transform ,sincPartnerSum = sum(sincNorm))
men = ddply(men, .(pid) ,transform ,intelPartnerSum = sum(intelNorm))
men = ddply(men, .(pid) ,transform ,funPartnerSum = sum(funNorm))
men = ddply(men, .(pid) ,transform ,ambPartnerSum = sum(ambNorm))
men = ddply(men, .(pid) ,transform ,sharPartnerSum = sum(sharNorm))
men = ddply(men, .(pid) ,transform ,likePartnerSum = sum(likeNorm))
men = ddply(men, .(pid) ,transform ,probPartnerSum = sum(probNorm))

women = ddply(women, .(pid) ,transform ,decPartnerSum = sum(decNorm))
women = ddply(women, .(pid) ,transform ,attrPartnerSum = sum(attrNorm))
women = ddply(women, .(pid) ,transform ,sincPartnerSum = sum(sincNorm))
women = ddply(women, .(pid) ,transform ,intelPartnerSum = sum(intelNorm))
women = ddply(women, .(pid) ,transform ,funPartnerSum = sum(funNorm))
women = ddply(women, .(pid) ,transform ,ambPartnerSum = sum(ambNorm))
women = ddply(women, .(pid) ,transform ,sharPartnerSum = sum(sharNorm))
women = ddply(women, .(pid) ,transform ,likePartnerSum = sum(likeNorm))
women = ddply(women, .(pid) ,transform ,probPartnerSum = sum(probNorm))


for(r in ratings){
  men[paste(r,"PartnerAvg",sep="")] = men[paste(r,"PartnerSum",sep="")]/men["numRaters"]
  women[paste(r,"PartnerAvg",sep="")] = women[paste(r,"PartnerSum",sep="")]/women["numRaters"]
}
...
for(r in ratings){
  men[paste(r,"PartnerAvgExclud",sep="")] =  (men[paste(r,"PartnerSum",sep="")] - men[paste(r,"Norm",sep=""] + men[paste(r,"FakeNorm",sep="")])/men["numRaters"]
  women[paste(r,"PartnerAvgExclud",sep="")] =  (women[paste(r,"PartnerSum",sep="")] - women[paste(r,"Norm",sep=""] + women[paste(r,"FakeNorm",sep="")])/women["numRaters"]
}
...
# ratings = c("dec", "attr", "sinc", "intel", "fun", "amb", "shar", "like", "prob")
# ownRatings = c("dec.x", "attr.x", "sinc.x", "intel.x", "fun.x", "amb.x", "shar.x", "like.x", "prob.x")
# otherRatings = c("dec.y", "attr.y", "sinc.y", "intel.y", "fun.y", "amb.y", "shar.y", "like.y", "prob.y")
# men_waves = hash()
# women_waves = hash()
# for(i in 1:1){
#   wave = merged_men[merged_men["wave.x"] == i,]  
#   for(r in ratings){
#     for(iid in unique(wave[["iid"]])){
#       wave[wave["iid"] == iid,][[paste("fake",r,sep=".")]] = sample(wave[wave["iid"] == iid,][[r]])
#     }
#   }
#   men_waves[[toString(i)]] = wave
# }
...
write.csv(merged_men, "~/Desktop/menSuperSuper.csv")
write.csv(merged_women, "~/Desktop/womenSuperSuper.csv")
