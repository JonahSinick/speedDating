...
features = c(baselineDecM, "attrAvgRatingW", "avgWaveDecW", "decAvgW", "likeAvgRatingW", "fieldsCrossProbTraitDecM",
             "goalsCrossProbTraitDecM", "racesCrossProbTraitDecM", "datesCrossProbMatch")
for(key in keys(frameHash)){
  df = frameHash[[key]]
  df["baselineProbDecM"] = getProbs(frameHash[["train"]], df, baselineDecM, "decM")
  for(name in newCrosses){
    print(name)
    df = frameHash[[key]]
...
round(futureTrainBaseline - 37 - 6 - 4 -3 - 26 - 4 - 10000*a)
round(futureTestBaseline - 3 -16 -6 -2 - 40 - 24 -  10000*b)
