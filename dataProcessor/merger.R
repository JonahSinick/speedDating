...
#   women = makeRatingMetrics(women)
  ...
  x_merges = c("iidW", "idW", "waveW" ,"partnerW", "pidW", "matchW")
  y_merges = c("pidM", "partnerM", "waveM", "idM", "iidM", "matchM")
  merged = merge(women, men, by.x = x_merges, by.y = y_merges)

  
  colnames(merged)[c(1,2,3,4,5,6)] = c("iidW", "idW", "wave", "idM", "iidM", "match")
