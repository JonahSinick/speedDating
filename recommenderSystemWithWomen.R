...
iidWMatrix = data.frame()
for(iidM in unique(wave7[["iidM"]])){
  iidWMatrix[toString(iidM),] = 0
  probMatrix[toString(iidM),] = 0
}
for(i in 1:16){
  iidWMatrix[paste("rec",toString(i),sep="_")] = 0
}
for(i in 1:16){
  iidWMatrix[paste("prob",toString(i),sep="_")] = 0
}

for(i in 1:16){
  iidWMatrix[paste("newrec",toString(i),sep="_")] = 0
}
...
for(j in 1:16){
  for(i in 1:16){
    for(iidM in unique(wave7[["iidM"]])){
      ...
      takens2 = iidWMatrix[iidM, newrecs]
      if(!(iidWMatrix[[iidM,recs[i]]] %in% takens1 | iidWMatrix[[iidM,recs[i]]] %in% takens2 | iidWMatrix[iidM,newrecs[j]] != 0 )){
