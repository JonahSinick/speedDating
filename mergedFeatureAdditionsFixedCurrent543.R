...
for(rating in menRatings){
  print(rating)
  slice[paste(rating,"Adj",sep="")] = 0
  for(iidW in unique(merged[["iidW"]])){
    slice[slice["iidW"] == iidW,][[paste(rating,"Adj",sep="")]] = slice[slice["iidW"] == iidW,][[rating]] - mean(slice[slice["iidW"] == iidW,][[rating]])
  }
}

for(rating in womenRatings){
  slice[paste(rating,"Adj",sep="")] = 0
  for(iidM in unique(slice[["iidM"]])){
    slice[slice["iidM"] == iidM,][[paste(rating,"Adj",sep="")]] = slice[slice["iidM"] == iidM,][[rating]] - mean(slice[slice["iidM"] == iidM,][[rating]])
  }
}


for(j in 1:length(womenRatings)){
  wr = gsub("$", "Adj", womenRatings[[j]])
...
n = names(merged)
menTraits = n[grep("M$",n)][c(3:51,78:111)]
