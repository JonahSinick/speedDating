...
for(key in keys(frameHash)){
  if(key == "test"){
    df = frameHash[[key]]
    df["baselineProbMatch"] = getProbs(frameHash[["train"]], df, c(baselineDecW, baselineDecM), "match")
...
matchProbs = c("match","baselineProbMatch", "featuresProbMatch")

matchAccuracyFrame = accuracyFrame(frameHash[["test"]][matchProbs],"match")
matchAccuracyFrame
