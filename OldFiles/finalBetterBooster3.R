...
populateProbs = function(newMerged, numTimes1, numTimes2){
  ...
  for(i in 1:10){
    ...
    for(j in 1:numTimes1){
      ...
      trainNewIdxs = sample(1:nrow(train))[1:floor(4*nrow(train)/5)]
    ...
    for(j in 1:numTimes2){
      print(c(i,j))
      train1Idxs = sample(1:nrow(train))[1:floor(4*nrow(train)/5)]
...
newMerged = populateProbs(merged, 10, 10)

newMerged["forestDecWGuess"] = 0
...
womenBase = c("raterDecLORW", "decLORM", "attrAvgRatingM", "fieldsCrossLORDecW", "attrAvgRatingAbsDiff",
              "datesCrossLORDecW", "goalsCrossLORDecW", "funAvgRatingAbsDiff", "raterDecLORM", "careersCrossLORDecW")
matchGuessFeatures = c("refinedConjProbCal", "attrAvgRatingAbsDiff", "goalsWomanLORDecM", 
                       "raterDecLORAbsDiff", "yogaPhysActM", "goalsCrossLORDecW")
train = newMerged[newMerged[["wave"]] %in% c(4,17,2,11,7),]
test = newMerged[newMerged[["wave"]] == 9,]
rf = randomForest(y=as.factor(train[,"match"]), x=train[matchGuessFeatures], importance=TRUE, ntree=500)
test["forestMatchGuess"] = predict(rf, test)
printMetrics(test[["match"]],test[[probs[8]]], cutoff=0.5)
table(test[["match"]],test[["forestMatchGuess"]])
