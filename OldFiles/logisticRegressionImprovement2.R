...
table(ifelse(round(test[["badDecMGuess"]],2) > 0.5,1,0),test[["decM"]])
matchGuessFeatures = c("naiveMatchGuessProb")
features = matchGuessFeatures
test[["matchGuess"]] =  getProbs(train, test, matchGuessFeatures, "match",tryCosts)
train[["matchGuess"]] =   getProbs(train, train, matchGuessFeatures, "match",tryCosts)
f(tryCosts, "match")
