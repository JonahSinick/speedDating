...
mainRatingsDecM = c("attrAvgRatingW",  "funAvgRatingW",  "likeAvgRatingW", "attrRatingGuessW",  "funRatingGuessW",  "likeRatingGuessW")
...
for(key in keys(h)){
  df = h[[key]]
  df["baselineProbDecM"] = getProbs(frameHash[["train"]], df, baselineDecM, "decW", tryCosts)
  df["baselineProbDecW"] = getProbs(frameHash[["train"]], df, baselineDecW, "decM", tryCosts)
  df["mainProbDecM"] = getProbs(frameHash[["train"]], df, menGuesses, "decW", tryCosts)
  df["mainProbDecM"] = getProbs(frameHash[["train"]], df, womenGuesses, "decM", tryCosts)
  h[[key]] = df
}
