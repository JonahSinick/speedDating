...
feature_hash[["whiteScore1"]] =n[grep("whiteAsian",n)]
...
for(k in keys(feature_hash)){
  ...
  for(string in c("dec_M")){
    print(string)
    rf_fit_x <- randomForest(y=as.factor(train[,string]), x=train[features], importance=TRUE, ntree=2000, m_try = length(features))
    predictions_x = predict(rf_fit_x, test, type="prob")
    predictions_col_x = matrix(predictions_x[,2])[,1]
    test[[k]] = predictions_col_x
    print(string)
    rf_fit_x <- randomForest(y=as.factor(train2[,string]), x=train2[features], importance=TRUE, ntree=2000, m_try = length(features))
    predictions_x = predict(rf_fit_x, test2, type="prob")
    predictions_col_x = matrix(predictions_x[,2])[,1]
    test2[[k]] = predictions_col_x
  }
  train = test2
  test = train2
}
