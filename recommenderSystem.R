...
finalPrintMetrics = function(waveHash){
  df = data.frame()
  df["placeHolder",] = 0
  oldNum = 0
  newNum = 0
...
optimized = optimizeParameter(merged, waves, 2,  "matchGuess", seq(0.3,1,by=0.05))
waveHash = makeWaveHash(merged, waves, 2, 0.85, "matchGuess")
df = finalPrintMetrics(waveHash)
optimized2 = optimizeParameter(merged, waves, 2, "genericConjProbCal", seq(0.2,1,by=0.05))
colSums(optimized2)
waveHash = makeWaveHash(merged, waves, 2, 0.75, "genericConjProbCal")
df = finalPrintMetrics(waveHash)
matches = df[df["match"] == 1,]
newSlice = matches[matches["betterOrder"] !=0 ,]
length(unique(newSlice[["iidM"]]))
length(unique(newSlice[["iidM"]]))
[1] 107
> length(unique(newSlice[["iidW"]]))
[1] 99
