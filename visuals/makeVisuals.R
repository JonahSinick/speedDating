...
mRatings = mRatings[1:8]
mRatings[c(2,3,8)] = mRatings[c(8, 2,3)]
...
wRaterAvgs = gsub("$", "RaterAvgW", traits)
mRaterAvgs = gsub("$", "RaterAvgM", traits)

mAvgs = gsub("$", "AvgM", traits)
avgs = gsub("$", "Avg", traits)
niceCors(merged,wRaterAvgs, "decW")
merged["comboRaterAvgW"] = rowSums(scale(merged[c("likeRaterAvgW", "attrRaterAvgW", "funRaterAvgW", "sharRaterAvgW")]))
n = names(merged)
prefs = n[grep("Pref",n)]
...
r = results(merged, c("decRaterAvgW", "comboRaterAvgW"), "decW", "test", "linear", thres=10)
attrLikeDecAvgWLow
