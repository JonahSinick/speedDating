...
colnames(men)[2:ncol(men)] = gsub("$", "M", names(men)[2:ncol(men)])
colnames(women)[2:ncol(men)] = gsub("$", "W", names(women)[2:ncol(women)])
men = men[2:ncol(men)]
women = women[2:ncol(women)]
...
merged = merged[!(names(merged) %in% c("XW", "XM"))]
