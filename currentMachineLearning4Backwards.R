...
futureTrainBaseline = 5316
futureTestBaseline = 5281
...
menFeatures = c(baselineDecM, "attrAvgRatingW", "avgWaveDecW", "decAvgW", "likeAvgRatingW", "fieldsCrossProbTraitDecM",
             "goalsCrossProbTraitDecM", "racesCrossProbTraitDecM", "datesCrossProbMatch", "fieldScienceM_fieldScienceWCross",
             "likeAvgRatingM", "sharAvgRatingM")
womenFeatures = c(baselineDecW, "likeAvgRating", "fieldsCrossProbDecW")
for(key in keys(frameHash)){
  df = frameHash[[key]]
  df["baselineProbDecW"] = getProbs(frameHash[["train"]], df, baselineDecW, "decM")
  for(name in c(newMenTraits,newWomenTraits, newCrosses)){
    df = frameHash[[key]]
    df[paste(name,"ProbDecW",sep="")] = getProbs(frameHash[["train"]], df, c(womenFeatures, name), "decW")
    frameHash[[key]] = df
  } 
  frameHash[[key]] = df
}
...
# b = accuracyFrame(frameHash[["futureTest"]][menProbs],"decM")
# round(futureTrainBaseline - 37 - 6 - 4 -3 - 26 - 4 - 1 -10  -11 - 17 -  10000*a["logLoss"])
# round(futureTestBaseline - 3 -16 -6 -2 - 40 - 24 - 10 + 9 -7   -8 - 10000*b["logLoss"])

n = names(frameHash[["futureTest"]])
probs = n[grep("Prob",n)]
womenProbs = c("decW", probs[grep("ProbDecW", probs)])
a = accuracyFrame(frameHash[["futureTrain"]][womenProbs],"decW")
b = accuracyFrame(frameHash[["futureTest"]][womenProbs],"decW")
5375 - round(10000*a["logLoss"])
5254 - round(10000*b["logLoss"])
