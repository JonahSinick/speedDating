...
createCrosses = function(df,menBinaries,womenBinaries, codeWords){
  for(codeWord in codeWords){
    ...
    for(m in mens){
      df[df[m] == 1,][[m]] = nrow(df[df[m] == 1,])
    }
    for(w in womens){
      df[df[w] == 1,][[w]] = nrow(df[df[w] == 1,])
    } 
    for(m in mens){
      for(w in womens){
        newName = paste(paste(m,w,sep=""),"Cross",sep="")
        df[newName] = 0 
        df[df[w] == 1 & df[m] == 1,newName] = nrow(df[df[w] == 1 & df[m] == 1,newName])
      }
    }
    for(m in mens){
      df[df[m] == 1,][[m]] = nrow(df[df[m] == 1,])
...
merged = merged[!(n %in% c("dateAfterW", "numDatesAfterW", "dateAfterM", "numDatesAfterM"))]

agged = aggregate(merged,by=merged[c("dateM","dateW")],FUN=mean)

agged = agged[agged[["dateM"]] %in% c(3,4,5,6,7) & agged[["dateW"]] %in% c(3,4,5,6,7), ]
agged[c("dateM","dateW", "decRatingM", "decRatingW")]
round(100*matrix(agged[["decRatingM"]],nrow = 5, ncol = 5) - 48)
